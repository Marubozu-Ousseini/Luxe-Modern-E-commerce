// Headless QA script (Puppeteer)
// Usage:
// 1) Install puppeteer: npm install --save-dev puppeteer
// 2) Start your dev server (e.g., `npm run dev`)
// 3) Run: node scripts/headless_qa_mobile_checks.js --url=http://localhost:5173

const puppeteer = require('puppeteer');
const argv = require('minimist')(process.argv.slice(2));
const URL = argv.url || process.env.FRONTEND_URL || 'http://localhost:5173';
...
(async () => {
  ...
  try {
    ...
    console.log('Found mobile search button, clicking...');
    ...
    console.log('Clicking first product image to open PDP...');
    await page.click('#catalogue img');
