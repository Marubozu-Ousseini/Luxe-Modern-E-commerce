...
# Optional: Firebase admin key secret (created only if provided)
resource "google_secret_manager_secret" "firebase_admin_key" {
  count     = length(var.firebase_admin_key_json) > 0 ? 1 : 0
...
resource "google_secret_manager_secret_version" "firebase_admin_key_v" {
  count       = length(var.firebase_admin_key_json) > 0 ? 1 : 0
  secret      = google_secret_manager_secret.firebase_admin_key[0].id
  secret_data = var.firebase_admin_key_json
}
